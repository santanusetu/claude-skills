...
`build_fixtures.py` writes 7 made-up sessions for a project at `/home/dev/acme-shop` into `fixtures/`, and the tests point the script at them with `CLAUDE_CONFIG_DIR`. Rebuild the fixtures with `python3 tests/session-finder/build_fixtures.py`.
...
```bash
CLAUDE_CONFIG_DIR="$PWD/tests/session-finder/fixtures" \
  python3 skills/session-finder/scripts/sessions.py --project /home/dev/acme-shop
```
